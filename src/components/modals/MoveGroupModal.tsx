...
import { X, Folder, ChevronDown, ChevronRight, Check, AlertCircle } from 'lucide-react';
...
export default function MoveGroupModal({
  ...
  const { knowledgeBases, groups, getChildGroups, getDescendantGroupIds } = useKnowledgeBaseStore();
  ...
  // Get forbidden group IDs (moving group itself + its descendants)
  const forbiddenGroupIds = React.useMemo(() => {
    if (!isOpen) return [];
    return [groupId, ...getDescendantGroupIds(groupId)];
  }, [isOpen, groupId]);

  // Calculate moving group's subtree height (max relative depth)
  const maxRelativeDepth = React.useMemo(() => {
    if (!isOpen) return 0;
  ...
  // Determine if moving to selected target is valid based on depth limits
  const targetParentGroup = groups.find((g) => g.id === selectedGroupId);
  const newDepthOfG = selectedKbId ? (selectedGroupId ? (targetParentGroup ? targetParentGroup.depth + 1 : 0) : 0) : 0;
  const isDepthLimitExceeded = newDepthOfG + maxRelativeDepth > 5;

  const handleConfirmAction = () => {
    if (selectedKbId && !isDepthLimitExceeded) {
  ...
  const GroupNode = ({ group, depth }: { group: Group; depth: number }) => {
    // If it's the moving group or its children, do not render it (to avoid circular move)
    if (forbiddenGroupIds.includes(group.id)) {
      return null;
    }

    const isExpanded = !!expandedNodes[group.id];
    // Get child groups, filtering out forbidden ones
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl shadow-2xl w-[480px] h-[540px] border border-border-color overflow-hidden flex flex-col p-6 animate-modal-scale-in"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex justify-between items-center mb-4 pb-2 border-b border-border-color shrink-0">
          <div>
            <h3 className="text-base font-bold text-text-primary">移动分组层级</h3>
            <p className="text-[11px] text-text-secondary mt-0.5">
              分组：<span className="font-semibold text-text-primary">“{groupName}”</span>
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto border border-border-color rounded-lg bg-bg-panel p-3">
          <div className="space-y-3">
            {visibleKBs.map((kb) => {
              ...
              // Filter out any root groups that are forbidden
                  ...
                  >
                    <div className="flex items-center gap-2 min-w-0 flex-1">
                      ...
                      <span className="truncate">
                        {kb.name} <span className="text-[10px] text-text-secondary font-normal">(根目录)</span>
                      </span>
                    </div>
        ...
        {/* Selected target info & alerts */}
        {selectedKbId && (
          <div className="mt-3 shrink-0">
            {isDepthLimitExceeded ? (
              <div className="px-3 py-2 bg-red-50 border border-red-100 text-red-600 rounded-lg text-xs flex items-center gap-2 font-medium">
                <AlertCircle size={14} className="shrink-0" />
                <span>目标位置层级过深，移动后将超出系统最大 6 层层级限制。</span>
              </div>
            ) : (
              <div className="px-3 py-2 bg-indigo-50/50 border border-indigo-100 rounded-lg text-xs text-text-secondary">
                准备移动至：
